...
def create_lot():
    if request.method=='POST':
        #form data of new lot details
        pl_name=request.form['prime_location_name']
        rate=float(request.form['price_per_hour'])
        address=request.form['address']
        pincode=request.form['pin_code']
        max_spots=int(request.form['max_spots'])
        new_lot=ParkingLot(
            prime_location_name=pl_name,
            price_per_hour=rate,
            address=address,
            pin_code=pincode,
            max_spots=max_spots
        ...
        db.session.commit()
        prefix = pl_name[:3].upper() if pl_name else "LOT"
        for i in range(max_spots):
            spot_num = f"{prefix}-{i+1:03d}"
...
def manage_lots():
    ...
    for lot in all_lots:
        spot_list=lot.spots
        total=lot.max_spots
        occupied=sum(1 for spot in spot_list if spot.status=='O')
        available=total-occupied
        lot_names.append(lot.prime_location_name)
        max_spots.append(total)
        occupied_count.append(occupied)
        available_count.append(available)
    return render_template('manage_lots.html', lots=all_lots, lot_names=lot_names, max_spots=max_spots, occupied_count=occupied_count, available_count=available_count)
...
def edit_lot(lot_id):
    ...
    if request.method=='POST':
        ...
        if new_max>lot.max_spots:
            ...
            spots_tbr=ParkingSpot.query.filter_by(
                lot_id=lot.id, 
                status='A'
            ).limit(lot.max_spots-new_max).all()
            for spot in spots_tbr:
...
def user_bookings():
    ...
    for r in reservations:
        if r.leaving_timestamp:
            m=r.parking_timestamp.strftime('%b-%Y')
            cost_by_month[m]=cost_by_month.get(m, 0)+(r.total_cost or 0)
    months=sorted(cost_by_month.keys())
    costs=[cost_by_month[m] for m in months]
    return render_template('user_bookings.html', reservations=reservations, now=datetime.utcnow(), user=user, lot_counts=lot_counts, user_lot_names=user_lot_names,user_bookings_counts=user_bookings_counts, months=months, costs=costs)
...
def get_lots():
    ...
    for lot in all_lots:
        ...
        lot_list.append(lot_info)
    return jsonify(lot_list)
...
def change_password():
    ...
    #Fetching the user-id from database
    user=User.query.get(session['user_id'])
    if not check_password_hash(user.password, current_pwd):
        flash('Oops! The current password you entered is incorrect!', 'danger')
